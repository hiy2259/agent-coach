...
`agent-coach`는 대상 프롬프트/스킬/지시문 파일을 마치 좋은 코치가 선수를 훈련시키듯 다듬는 [Claude Code](https://docs.claude.com/en/docs/claude-code) 스킬입니다(스킬 본체는 [`skills/agent-coach/`](./skills/agent-coach)에 있습니다):
...
가장 중요한 구조적 규칙 하나: **변경을 제안하는 모델은 그것을 채점하는 모델이 아니며, 둘 중 어느 것도 대상을 실행하는 모델이 아니다.** 각 행위자는 [`skills/agent-coach/agents/`](./skills/agent-coach/agents) 아래에 자기 프롬프트를 가진 별도의 Claude Code 서브에이전트입니다. 그래서 각자 깨끗한 컨텍스트에서 시작하고, 역할 분리가 명목상이 아니라 *실제로* 보장됩니다.
...
판단은 모델이 아니라 [`score_compare.py`](./skills/agent-coach/scripts/score_compare.py)가 내립니다. 변경은 **다음이 모두 성립할 때만 MERGE 됩니다:**
...
마진 `eps_train` / `eps_heldout`은 **측정 노이즈**이며, [`calibrate_noise.py`](./skills/agent-coach/scripts/calibrate_noise.py)가 보정합니다: 고정된 입력에 대해 실행자를 `k_calib`번(5 이상 권장) 다시 돌리고, 각각 채점해서, 분할별 점수 산포를 도출합니다 — 작은 양수 `min_eps`로 바닥을 깔아서 `+0.0` 동점이 진보처럼 보이는 일을 막습니다. 홀드아웃 마진은 **대칭(symmetric)**입니다(병합 쪽과 HALT 쪽 모두 `eps_heldout`). 그래서 평범한 홀드아웃 노이즈가 거짓 HALT를 유발하지 않습니다.
...
이 불변식들이 바로 이 스킬이 존재하는 이유입니다. 각각은 "자기개선"이 측정 없는 진화로 조용히 퇴락하는 특정한 한 가지 경로를 제거합니다. 전체 서술: [`references/safety-invariants.md`](./skills/agent-coach/references/safety-invariants.md).
...
→ **전체 walkthrough** — 세 가지 시나리오(웜/콜드/과적합 방어), 중단된 실행 재개, 콜드 스타트, 비용 — 은 [`docs/agent-coach/running.ko.md`](./docs/agent-coach/running.ko.md)에 있습니다.
...
📖 상세 사용법 가이드는 [`docs/agent-coach/`](./docs/agent-coach)에 있습니다 — 영어와 한국어(`*.ko.md`)로:

| 가이드 | 다루는 내용 |
|---|---|
| [**좋은 골든셋 만들기**](./docs/agent-coach/golden-set.ko.md) | 골든셋은 해자입니다: 케이스 고르기, 루브릭 작성 craft, train/held-out 분할, 보정, 실행 사이 셋 진화, 안티패턴, 워크드 예제. |
| [**실행 설정하기**](./docs/agent-coach/run-config.ko.md) | `run-config.json`의 모든 필드, 측정을 정직하게 유지하는 3가지 규칙, 사전 점검. |
| [**루프 실행하기**](./docs/agent-coach/running.ko.md) | 빠른 시작, 세 가지 실전 시나리오, 중단된 실행 재개, 콜드 스타트, 비용. |

영문: [`golden-set.md`](./docs/agent-coach/golden-set.md) · [`run-config.md`](./docs/agent-coach/run-config.md) · [`running.md`](./docs/agent-coach/running.md).

골든셋이 아직 없다면? 동반 스킬 [`golden-set-drafter`](./skills/golden-set-drafter/SKILL.md)가 초안을 만들어 줍니다(회의체 검토 + 실패 실측). 단, held-out 채점표를 **사용자가 직접** 작성해야 열리는 게이트에서 멈춥니다 — [`docs/golden-set-drafter/`](./docs/golden-set-drafter/README.ko.md) 참조.

---

## 저장소 구조

README는 프로젝트 루트에 있습니다. 이 저장소는 `skills/` 아래에 **두 개의 스킬**을 담습니다 — `agent-coach`(이 README의 주제)와, agent-coach*를 위한* 시작 골든셋 초안을 만들어 주는 동반 스킬 [`golden-set-drafter`](./skills/golden-set-drafter/SKILL.md). `docs/`와 `examples/`는 스킬당 한 폴더로 구분됩니다.
...
│   │   └── …                          #   상세 구조는 아래
│   └── golden-set-drafter/            # 동반 스킬: agent-coach용 골든셋 v1 초안 작성
│       ├── SKILL.md                   #   골든셋이 없을 때 agent-coach보다 먼저, 그 대신 실행됨
│       ├── agents/                    #   회의체 행위자(gsd-proposer/-adversary/-arbiter) + vendored/ runner·grader 사본
│       ├── scripts/                   #   emit_draft.py · scan_inputs.py + 테스트(드리프트 가드 포함)
│       ├── references/                #   heldout-rubric-guide.md — 게이트 앞의 사람을 위한 작성 가이드
│       ├── evals/                     #   drafter 자체에 대한 레벨-2 eval
│       └── assets/                    #   runbook-template.md — 방출되는 초안 README의 골격
└── loop/                              # 실행별 작업 상태 (아래 설명 참조)

skills/agent-coach/                    # 스킬 본체
...
코드가 모든 되돌릴 수 없는 결정을 내립니다; 모델은 오직 생성하고 채점만 합니다. 모든 스크립트는 [`skills/agent-coach/scripts/`](./skills/agent-coach/scripts) 아래에 있습니다.
...
- `skills/agent-coach/scripts/`를 위한 **Python ≥ 3.8** — **표준 라이브러리만** 사용, 서드파티 패키지 없음.
