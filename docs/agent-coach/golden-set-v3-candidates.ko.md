...
이 스킬을 자기 자신에게 적용해 시험할 때 쓰는(이른바 dogfood) 골든셋은 **v2로 동결**된
...
- **동결·검증이 끝난 셋을 건드릴 명분이 없습니다.** 실무에서 c4는 이미 수용 가능한
  ...
  동결·검증된 골든셋은 실제 사용자 가치가 있을 때만 수정해야 합니다. "문구가 더 깔끔해질
  수 있다"는 그런 가치가 아닙니다.
...
1. 고정된 채점자(`version_id` 2026-06-19)로 **라이브** dashboard-slow 출력의 인덱스 3을
   ...
   위험한 이유입니다.
2. 라이브 인덱스 3이 FAIL이라면(즉 그것이 그 개선 여지라면), 먼저 train의 포화를 풀어
   주세요. 더 어려운 "지어내기 축" 케이스를 추가해 train 점수가 1.0 아래로 내려오게 만든
   다음, 그 후에야 문구를 강화합니다. 그러지 않으면 게이트가 통과 불가능해집니다.
3. 대기 중인 "인덱스 5 / 형식 비포화" 후보와 **묶어서 처리하지 마세요.** 그 후보는 서로
   무관한 축(출력 형식)에 부담을 주는 것이라 c4의 상한을 풀어 주지 못하고, 케이스를
   사람이 직접 골라 다듬어야 하는 S5 부담도 따로 생깁니다.

### 바탕에 깔린 채점자 불일치의 진짜 안전망

인덱스 3에 대한 세션 간 판정 불일치는 실재합니다. 하지만 그 안전망은 c4 문구 강화가
아니라 **자동 감시 장치**입니다. 서로 다른 모델 계열의 채점자 둘(dual judge)에게 같은
출력을 채점시키고, 판정이 갈리면 경보를 울리는 방식(이른바 cross-family tripwire)입니다.
그 첫 단계는 2026-06-25에 출시되었습니다. 동작은 바꾸지 않고 판정 불일치를 기록만 하는
경고(`scripts/check_cross_validation.py`)입니다. 판정을 실제로 비교해 조치까지 하는 본체
자동화(comparator)는 포화가 풀린 진단 셋이 마련될 때까지 의도적으로 보류(HOLD)
