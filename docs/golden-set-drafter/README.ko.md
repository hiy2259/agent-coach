...
| 산출물 | 내용 | 예시 |
|---|---|---|
| `golden-set.json` | train(입력+채점표 완성) + held-out(입력만, **채점표 공란**), 미동결 | [`draft-output/golden-set.example.json`](../../examples/golden-set-drafter/draft-output/golden-set.example.json) |
...
| `GOLDEN-SET-DRAFT-RUNLOG.json` | 회의체·자(ruler)·expose 기록 | [`draft-output/GOLDEN-SET-DRAFT-RUNLOG.example.json`](../../examples/golden-set-drafter/draft-output/GOLDEN-SET-DRAFT-RUNLOG.example.json) |
...
- 대신 agent-coach의 `op=split`이 **코드로** 사람 작성을 강제합니다: 채점표가 빈 채로
  ...
  출력: [`gate-first-run.example.json`](../../examples/golden-set-drafter/gate-first-run.example.json).
...
| [`examples/golden-set-drafter/`](../../examples/golden-set-drafter/draft-output/golden-set.example.json) | 방출물 예시 (KO / `en/` 미러) |
