...
| 입력 | 필수 | 설명 |
|---|---|---|
| **대상 파일** | 예 | 골든셋을 만들 프롬프트/스킬/지시문 파일 경로 — 예: `./agents/support-agent.md`. |
| **발전 방향** | 예 | 대상이 *무엇을 더 잘해야 하는가* — 목표·실패 사례·로그·수집된 경험. 자유 텍스트 또는 파일, 예: `./direction.md`. 회의체가 어떤 케이스를 초안할지 좌우합니다. |
| **기존 골든셋** | 아니오 | 기존 세트를 주면 v1은 "update/evolve는 v2 기능"이라고 밝히고 **정지**합니다 — 언제나 **새 v1**만 초안하며 절대 반쯤 갱신하지 않습니다. |

또한 대상의 `run-config.json`을 자동으로 읽어 **운영 모델 + temperature**를 맞춥니다 — train 실패를 실측하는 "자(ruler)"이며, 자가 틀리면 초안이 포화된 채 나옵니다. run-config가 없으면? 대화형에서는 실제 모델/temperature를 **물어보고**, 헤드리스에서는 하나를 스캐폴딩하며 자를 *가정(assumed)*으로 표시합니다. 전체 옵션 레퍼런스: [run-config](../agent-coach/run-config.ko.md).
