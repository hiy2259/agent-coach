...
> 대상 파일과 발전 방향 문서로부터 골든셋 첫 버전(v1)의 초안을 만들어 줍니다. 단,
...
스킬 계약 전문: [`skills/golden-set-drafter/SKILL.md`](../../skills/golden-set-drafter/SKILL.md)
...
agent-coach는 골든셋(시험지)이 있어야 돌 수 있는데, 골든셋 규칙을 아직 모르는 사용자가
처음부터 직접 만들기는 어렵습니다. 이 스킬이 그 무거운 부분을 세 단계로 대신합니다:

1. **회의체 초안.** AI 세 역할 — proposer(제안) → adversary(공격) → arbiter(판정) — 가
...
2. **실패 노출.** 모든 train 케이스를 **실제 대상 파일 + 실제 운영 모델**로 한 번씩 돌려
   ...
   들여다볼 수 없습니다.)
3. **미동결 출력.** 나오는 것은 완성본이 아니라 **초안**입니다: held-out 채점 기준은 전부
   공란이고, `split_hash`도 없습니다. 그 공란을 사람이 채우도록 agent-coach의 코드
   게이트가 강제합니다.

골든셋이 없는 상황이라면, 올바른 순서는 이 스킬을 agent-coach**보다 먼저, agent-coach
대신** 실행하는 것입니다.
...
| 입력 | 필수 | 설명 |
|---|---|---|
| **대상 파일** | 예 | 골든셋을 만들어 줄 프롬프트/스킬/지시문 파일의 경로 — 예: `./agents/support-agent.md`. |
| **발전 방향** | 예 | 대상이 *무엇을 더 잘하게 되어야 하는가*: 목표, 실패 사례, 로그, 쌓아 온 경험. 자유 텍스트도 되고 파일도 됩니다(예: `./direction.md`). 회의체가 어떤 케이스를 초안할지 이것이 좌우합니다. |
| **기존 골든셋** | 아니오 | 기존 셋을 건네면 v1은 "갱신(update/evolve) 모드는 v2 기능"이라고 답하고 **멈춥니다**. 언제나 새 v1만 초안하며, 기존 셋을 어중간하게 고치는 일은 절대 하지 않습니다. |

또한 대상의 `run-config.json`을 자동으로 읽어 **운영 모델 + temperature**를 맞춥니다 —
train 실패를 잴 때 쓰는 "잣대"입니다. (잣대가 틀리면 초안이 포화된 채로 나옵니다: 전부
멀쩡해 보여서 아무것도 배울 수 없게 됩니다.) run-config가 없다면? 대화형 세션에서는 실제
모델과 temperature를 **직접 물어보고**, 무인(headless) 실행에서는 기본 틀을 만들어 두되
잣대가 *가정된(assumed)* 값임을 표시합니다. 전체 옵션 레퍼런스:
...
| 산출물 | 내용 | 예시 |
|---|---|---|
| `golden-set.json` | train 케이스(입력 + 채점 기준 완성) + held-out 케이스(입력만, **채점 기준은 공란**), 미동결 상태 | [`draft-output/golden-set.example.json`](../../examples/golden-set-drafter/ko/draft-output/golden-set.example.json) |
| `GOLDEN-SET-DRAFT-README.md` | 사용자의 케이스 용어로 쓰인 런북: 다음에 할 4단계 + **정직한 한계 10가지** + 정확한 `op=split` 명령이 담긴 "Gate data" 부록 | 골격: [`runbook-template.md`](../../skills/golden-set-drafter/assets/runbook-template.md) |
...
- **held-out 채점 기준을 어떤 형태로도 쓰지 않습니다.** 예시도, 출발점도, "초안만
  살짝"도 거절합니다 — 스킬 계약의 §5-2 규칙이고, 예외는 없습니다. 이 거절 자체가
  안전 설계의 일부입니다.
- 강제는 **코드**에 맡깁니다: held-out 채점 기준이 하나라도 비어 있으면 agent-coach의
  `op=split`이 실패하고, 에러 메시지에 비어 있는 케이스 id를 정확히 나열합니다. 그 에러가
  곧 게이트입니다 — 실제로 캡처한 실행 결과:
  [`gate-first-run.example.json`](../../examples/golden-set-drafter/ko/gate-first-run.example.json).
...
2. **held-out 채점 기준을 전부 직접 씁니다.** 이 단계의 가이드는
   ...
   하나를 못 박는 대신 좋은 답의 성질을 적어야 하며 — 가드 기준은 한 케이스에 몰지 말고
   여러 케이스에 걸쳐 배치합니다.
3. 런북의 "Gate data" 부록에 있는 **`op=split` 명령을 실행합니다**. 첫 실행은 **실패하는
   것이 정상**입니다 — 위에서 설명한 게이트가 제 역할을 하고 있는 것입니다.
4. 공란을 다 채운 뒤 다시 실행하면 셋이 **동결**됩니다. 그다음은 agent-coach의 보정이
   이 셋으로 측정이 가능한지 판정하고, [개선 루프](../agent-coach/running.ko.md)가
   이어받습니다.

## 범위와 정직한 한계

- **v1은 새 셋 초안 전용입니다.** 기존 골든셋을 입력으로 주면 "갱신 모드는 v2 예정"임을
  밝히고 **멈춥니다** — 어중간한 갱신은 절대 하지 않습니다.
- 실패 노출 증거는 **train 전용의 참고 지표**입니다. 이 셋으로 실제 실행이 가능한지
  (변별력/포화)의 최종 판정자는 agent-coach의 보정 단계입니다.
- train은 입력과 채점 기준 모두 AI가 작성합니다 — 의도적으로 감수한 v1의 트레이드오프입니다.
  이를 봉인하는 것이 사람이 소유한 held-out 채점 기준과 agent-coach의 S1 과적합 HALT입니다.
- 함께 나오는 모든 런북에는 **정직한 한계 10가지 전문**이 실립니다 — 출력을 만드는 코드가
  번호 항목 수를 직접 세고, 10개보다 적으면 런북 생성을 거부합니다.
...
| [`../agent-coach/golden-set.ko.md`](../agent-coach/golden-set.ko.md) | 골든셋 만들기 일반론 — 이 스킬이 준거로 삼는 문서 |
